...
from app.domain.exceptions import InvalidCredentialsError, InvalidTotpError, RateLimitedError
...
async def test_unknown_login_still_burns_a_hash() -> None:
    ...
    use_case, hasher = build(FakeUsers())
...
async def test_recovery_code_is_single_use() -> None:
    ...
    users = FakeUsers([make_user()])
    use_case, _ = build(users, invitations)
...
async def test_recovery_code_matches_regardless_of_formatting() -> None:
    ...
    use_case, _ = build(FakeUsers([make_user()]), invitations)

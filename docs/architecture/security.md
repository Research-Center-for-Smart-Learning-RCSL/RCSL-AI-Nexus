...
One number remains unverified and bears on this row: whether the OS actually evicts those file-backed pages under pressure rather than merely being free to, which needs a deliberate allocation on a serving machine and is a decision rather than a measurement. So does whether headroom survives a request at the 65536-token ceiling, where context cost is superlinear.
...
- The **gateway** account may read every table except one, and may INSERT into `usage_records` and `prompt_logs`, nothing else. It cannot write `api_keys`, `routing_policies`, or `users`, so a compromised public service cannot mint itself an admin key. The gateway does need INSERT on those two, so "read-only" is the wrong shape: the restriction is per table, and the writable set is named in code (`GATEWAY_WRITABLE_TABLES`) where it is subject to review.
...
- **`prompt_logs` is the one table worth considering excluding**, added here with §9.2's full-text logging. Its retention ceiling is thirty days precisely so the platform does not accumulate a corpus of unpublished ideas; a backup that keeps every nightly snapshot would restore exactly that accumulation on a different disk, outliving the window by however long backups are kept. Either exclude the table or make the backup retention shorter than the dataset's, and say which — this is the kind of interaction that is obvious when written down and invisible when not.

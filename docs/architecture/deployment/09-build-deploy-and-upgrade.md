...
**Restarting Docker Desktop reaches the same state and no reconciler covers it.** On 2026-08-25 `docker desktop restart` stopped all twelve containers cleanly — eleven `Exited (0)`, `qdrant` `Exited (143)` — and restored none, leaving the platform entirely down until `docker compose up -d` was run by hand. `restart: unless-stopped` is not at fault and is not a second chance here: a restart stops containers *explicitly*, which is exactly the "unless", so the daemon has no event to act on when it returns. The state is the 19:10 boot's, but the trigger is not a boot, and everything written for that state is boot-shaped — the reconciler is a LaunchDaemon that runs at startup, and both injectors that rehearse it work by rebooting. An operator restarting Docker Desktop mid-session therefore gets no reconciler at all. What does cover it is `check-platform-health.sh` on its five-minute interval, which bounds the outage at five minutes plus however long the mail takes to be read; that is the only net under this case, which the port-binding reconciler's name rather obscures.

...
| `NODE_TOTAL_MEMORY_GB` | `54` | What is available to **models**, which stopped being the machine's 64 GB on 2026-09-05: the Colima migration put the containers inside a VM whose host footprint is 9.3 GB, outside this figure and outside the fifth `MemoryBudgetService` reserves for "the OS, the containers, and inference working memory" — a sentence written when the containers were not behind a hypervisor. Measured 2026-09-07 with 39.05 GB of models resident: 0.8 GB free, 13.5 GB compressed, 6.9 GB of swap, and the budget still reporting 12.2 GB available. 54 × the 0.8 headroom is 43.2 GB: above the 48.8 the current three models need to stay reloadable, below the 47.5 GB `qwen36-35b-a3b-q8` beside them would take. Too high lets the guardrail walk the host into swap; too low refuses models that would fit ([ARCHITECTURE.md](../../ARCHITECTURE.md) §0.2) |

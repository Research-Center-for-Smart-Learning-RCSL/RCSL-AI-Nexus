...
| `NODE_TOTAL_MEMORY_GB` | `64` | The machine's unified memory, with nothing netted out of it — it is the `nodes` row's `total_memory_gb`, so it answers what the host *has*. Read together with the row below, whose product is the budget `MemoryBudgetService` refuses loads against. It said `54` for four hours on 2026-09-07, netting the Colima VM out here instead; that put a deduction in a figure whose name and column say hardware, stacked it under the fifth the fraction already reserved, and the resulting 43.2 GB budget made `gemma4-31b-q8` unloadable — taking `code`, whose only candidate it is, down with it ([ARCHITECTURE.md](../../ARCHITECTURE.md) §0.2) |
| `NODE_MEMORY_HEADROOM_FRACTION` | `0.80` | The share of it models may occupy, and where a host's non-model footprint is declared. It equals `MemoryBudgetService`'s own default, and is a setting anyway because for two days it could not be: the 2026-09-05 Colima migration put the containers inside a VM whose host footprint measured 9.4 GiB on 2026-09-07, against a fifth sized for "the OS, the containers, and inference working memory" on a host running them natively. **The VM's 3 GiB cap is load-bearing for this figure** — the containers use 1.3 GiB inside the guest, and the host footprint is that allocation plus the guest page cache that grows to fill it; at the original 6 GiB it reached 9.4 and Ollama evicted a model on every load ([01-physical-components.md](./01-physical-components.md)). What llama.cpp reports allocating: `gemma4:31b-it-q8_0` at `n_ctx` 262144 is 30.38 + 11.25 + 2.40 = **44.02 GiB**, `qwen2.5:7b` at 32768 is **5.32**, `nomic-embed-text` at 2048 is **0.30**. 64 × 0.80 = 51.2 against the 51 those three are registered at: **0.2 GiB of margin, which is the host speaking rather than a choice** — the model set is within 0.4% of what the machine holds. Measured with all three resident: 0.09 GiB free, 4.44 GiB compressed, swap flat at 375 MB over 80 seconds, every service healthy. Too high lets the guardrail walk the host into swap; too low refuses models that would fit, and at 0.70 it refused the only candidate `code` has |
...
| `QUEUE_WAIT_SECONDS` | `1200` | How long a request may wait for a slot before `503 overloaded` with `Retry-After`. It exists because of the number two rows down: a slot can legitimately be held for 35 minutes, and before 2026-08-05 a caller arriving with every slot taken waited that long producing zero bytes and no code, which is indistinguishable from a hung deployment. §6 makes this class of setting the only line of defence, and a guardrail that refuses silently is half a guardrail. Zero or negative restores the unbounded queue. Raised from 120 on 2026-09-05: the runtime slot is `-np 1`, so a second heavy user is *going* to wait, and refusing them after two minutes while the first still has fifteen to run is a worse answer than the wait |

...
**Model runtimes are not containers.** Ollama and MLX run natively on macOS under launchd, bound to `127.0.0.1`, because Docker on macOS cannot reach the GPU. Containers connect through `host.docker.internal:11434`. Rationale in [ARCHITECTURE.md](../../ARCHITECTURE.md) §0.1; host-level hardening in [security.md](../security.md) §7.1(d). Ollama is a Homebrew formula, **0.33.2 since 2026-09-02**, and its version is not an implementation detail: the same model at the same context measured 31.58 GiB resident under 0.32.4 and 33.55 under 0.33.2, which is 1.97 GiB off the memory budget's headroom. The upgrade procedure, the window in the middle of it where the runtime answers but cannot load, and the fact that it has no local rollback are in [§9](./09-build-deploy-and-upgrade.md).

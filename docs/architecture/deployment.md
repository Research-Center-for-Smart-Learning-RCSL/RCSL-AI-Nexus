...
**What decides that race is `tailscaled`'s own startup, and it varies by more than the whole margin.** Two boots on 2026-07-26 bracket it: on the failing one the address reached `utun0` at 16:45:32 while Docker bound at 16:45:29, three seconds too early; on the next, `utun0` at 17:21:48 against Docker at 17:21:59, eleven seconds to spare. The seventeen-second difference is `tailscaled` stalling in a logtail bootstrap-DNS retry loop before it runs its state machine — `dial "log.tailscale.com:443" failed: no such host`, then bootstrap attempts against several DERP hosts in turn — which is precisely what a cold boot with no DNS yet produces, so the failing case is the ordinary path rather than an anomaly. The address needs neither the network nor the control plane: it is restored from the cached prefs in `/Library/Tailscale`, and on the second boot `utun0` carried it while `tailscaled` was still reporting `You are logged out`. The ordering therefore cannot be relied on in either direction, and the reconciler below is the only thing that covers a lost race.

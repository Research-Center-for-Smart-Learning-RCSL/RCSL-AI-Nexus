...
**What decides that race is `tailscaled`'s own startup, and it varies by more than the whole margin.** Five boots on 2026-07-26 bracket it. Docker is the stable side, binding 11 to 14 seconds after `tailscaled` starts on every boot where it bound at all; the entire variance is how long the address takes to arrive.
...
**The variable is whether the netmap disk cache loads.** With it, the address is up in the same second `tailscaled` starts, because it needs neither the network nor the control plane — at 17:21:52 the daemon was still reporting `You are logged out ... failed to resolve controlplane.tailscale.com` with the address already on `utun0`. Without it, the address waits for control: 9 seconds on the third boot, 17 on the failing one. The cache is written when a new netmap arrives from control, and a boot that *loads* it does not rewrite it, so caches do not chain: a boot that wins by eleven seconds leaves nothing behind and hands the next one the slow path. Applying a tailnet ACL also invalidates it, since the netmap carries the packet filter. The model has made two predictions and both held: 18:08:23 wrote the cache, so the 19:09 boot should have been a fast one, and it loaded the cache with the address up at +1 second; 19:09 loaded without rewriting, so the 19:43 boot should have been a slow one, and it found `netmap cache is not available`, waited 9 seconds, and wrote a fresh cache at 19:43:38. By the same rule the boot after that one is fast again.

**What the model does not buy is a losing boot on demand.** Both cache-miss boots on record still passed, by +2s and +2.7s; only 16:45 actually lost, and the 17-second address that made it lose has not recurred. "The slow kind" empirically means "wins by two seconds", so back-to-back reboots raise the odds of exercising the binding repair and do not force it.
...
The ordering therefore cannot be relied on in either direction, and the reconciler below is the only thing that covers a lost race. One practical consequence: to exercise the reconciler's repair path deliberately, reboot twice in a row and watch the second boot, which is the one that will have no cache.

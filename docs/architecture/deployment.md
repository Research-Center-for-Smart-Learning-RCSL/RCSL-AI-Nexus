...
**What decides that race is `tailscaled`'s own startup, and it varies by more than the whole margin.** Three boots on 2026-07-26 bracket it. Docker is the stable side, binding 11 to 14 seconds after `tailscaled` starts on all three; the entire variance is how long the address takes to arrive.
...
**The variable is whether the netmap disk cache loads.** With it, the address is up in the same second `tailscaled` starts, because it needs neither the network nor the control plane — at 17:21:52 the daemon was still reporting `You are logged out ... failed to resolve controlplane.tailscale.com` with the address already on `utun0`. Without it, the address waits for control: 9 seconds on the third boot, 17 on the failing one. The cache is written when a new netmap arrives from control, and a boot that *loads* it does not rewrite it, so caches do not chain: a boot that wins by eleven seconds leaves nothing behind and hands the next one the slow path. Applying a tailnet ACL also invalidates it, since the netmap carries the packet filter. The model made one prediction and it held: 18:08:23 wrote the cache, so the 19:09 boot should have been a fast one, and it loaded the cache with the address up at +1 second. By the same rule the boot after it is a slow one.

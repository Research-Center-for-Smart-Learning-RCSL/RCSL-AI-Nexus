...
Docker 那一側在前三次是穩定的：都在 `tailscaled` 起來後 11～14 秒綁定。**那三次全部的
變數都在「位址多久上來」**——0、9、17 秒。預算大約十一秒。
...
**預測對了。** 19:09:59 那次開機讀到了快取（上表最後一列），位址 +1 秒就上來，是快的那
一種。而依照同一條規則，它讀了快取就不會重寫，所以**再下一次開機會是慢的那種**——也就
是說，下一次重開正好是逼出 `OK: all bindings restored` 機率最高的那一次。這是兩個觀察撐
著的模型了，不是一個；還不算機制，但它連續預測對了一次。
...
**狀態現在是：第一輪通過三次，第二輪跑了一次、失敗。** 第二輪的失敗不是第一輪的失敗，
第一輪的三次通過仍然成立；但「兩輪都過」還沒有發生，所以下面那句「兩輪都過才代表這台
機器真的能自己復原」仍然是未達成的。兩條修復路徑也都還沒有被任何一次開機真的走過：
binding 修復路徑一次都沒觸發，容器拉起路徑是在 19:09 那次失敗之後才寫的。

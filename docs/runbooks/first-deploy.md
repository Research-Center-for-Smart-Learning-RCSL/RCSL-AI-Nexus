...
**這個修法是分開驗的，還沒有在真實開機上看過。** grace 那條路是把 `BOOT_GRACE` 調到大於
當時 uptime 跑出來的（機器已經開了二十分鐘，沒有辦法不重開就回到「開機後五分鐘內」）；
`RunAtLoad` 那一半是 20:53 重載時驗的，它確實觸發、確實寫了 state 檔，而因為 uptime 早就
超過 240 秒，它正確地走了**完整檢查**那條路並且沒寄信。**沒被看過的是兩者在開機時一起
動作**：`RunAtLoad` 落在 grace 窗內、走無聲重寫，五分鐘後的排程觸發才做真檢查。所以下一次
開機要多看一件事——**state 檔的 mtime 應該在開機後幾秒內就是新的，而且信箱是空的**。
...
天花板是 1.3 秒而它從來沒有變成負的（下面「快取不會接續」那段是完整的算式）。要真的走過
binding 修復路徑，用 **§1.1a 的故障注入**，不要一直重開。
...
`10.3 − 9 = 1.3 秒`。**這個槓桿的天花板就是 1.3 秒的餘裕，靠它再重開下去不會輸。** 唯一
真的輸過的 16:45 是位址等了 17 秒，而那個 17 秒在之後六次開機裡一次都沒有再出現。「慢的
那種」現在的實測意思是「贏 1.4 到 2.7 秒」，不是「會輸」。

**所以停止用重開機碰運氣。** 要填上那一格，改用下面 §1.1a 的故障注入。

**狀態現在是：第一輪通過六次，第二輪跑了一次、失敗。** 第二輪的失敗不是第一輪的失敗，
第一輪的六次通過仍然成立；但「兩輪都過」還沒有發生，所以下面那句「兩輪都過才代表這台
機器真的能自己復原」仍然是未達成的。**第二輪至今只跑過一次，而那一次失敗之後，把它救起來
的那條路徑（容器拉起）從來沒有在開機時被走過**——這是現在最欠的一次測試，因為 19:09 正是
它存在的理由。兩條修復路徑也都還沒有被任何一次開機真的走過：
...
六次、第二輪失敗一次，所以這句話還沒有成立。在它成立之前不要遠端做系統更新——更新失敗
停在互動畫面時，遠端修不了。
...
```
recreating: gateway admin-public frontend-public
OK: all bindings restored
```

那就是第二種結果，**表上空了七次開機的那一格**。接著把 §1.1 的完整檢查跑一遍確認平台真的
回來了：九個服務、六個 binding requested 與 actual 相符、六個入口 200。
...
**跑完之後，這一節的紀錄要更新**：注入不是自然發生的開機，所以 §1.1 的「六種結果」表上
那一格要註明是注入出來的，不是等到的。

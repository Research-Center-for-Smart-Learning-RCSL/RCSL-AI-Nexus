...
**最後那一列不屬於上面的統計，它是造出來的天氣。** 它不能拿去算 Docker 的分布或位址的
分布——`tailscaled` 不是自然起的，位址那一欄量的是「釋放之後多久」不是「開機之後多久」。
它唯一要說的是餘裕：自然開機的天花板是 +1.3 秒，注入把它推到 **−78 秒**，六十倍。這正是
90 秒那個數字要買的東西，而 21:02:56 那三條綁失敗就是買到了的證據。
...
**所以停止用重開機碰運氣。** 要填上那一格，改用下面 §1.1a 的故障注入——2026-07-26 21:02
照做了，那一格當場就填上了。
...
次」）本來就對得出來。**第二輪至今只跑過一次，而那一次失敗之後，把它救起來
的那條路徑（容器拉起）從來沒有在開機時被走過**——這是現在最欠的一次測試，因為 19:09 正是
它存在的理由。

**兩條修復路徑，現在一條走過了、一條還沒有。**

...
[← 首次部署 runbook](./first-deploy.md)

這裡是 [`first-deploy.md`](./first-deploy.md) 的 **§1.1、§1.1a、§1.1b**，連同它們
共用的那段「為什麼人一定要在現場」。**編號原封不動**，因為 `ROADMAP.md` 和進度日誌
一共有八處以「runbook §1.1」「runbook §1.1a」「runbook §1.1b」的形式引用它們。

搬出來的理由是它們本來就不屬於「第一次部署」的流程：§1.1 自己的標題就寫著「等第 7
部分整套跑起來之後再做」，而 §1.1a／§1.1b 是每次改動開機路徑之後要重跑的驗收，不是
...
**那個 boot grace 在 2026-07-26 之前一次都沒有生效過。** 它解析
...
（那次重啟一併推翻了 §1.1 的 netmap 快取模型，見上面那一段。）
...
- **監測沒有寄信，而那是對的。** 21:02:56 到 21:05:31 這 2 分 35 秒平台是真的壞的（三個
  ...
  `sed` 之前它一次都沒生效過，而在那之後也一直沒有壞掉的開機可以測它。如果
...
**這次注入一併推翻了一個跟它無關的數字，而且是最有價值的收穫。** 它壓的是 Docker 那一側，
...
**一個附帶確認的結果，記在這裡因為它很容易被讀反。** 完整檢查第一次是 21:53:29 手跑的，開機

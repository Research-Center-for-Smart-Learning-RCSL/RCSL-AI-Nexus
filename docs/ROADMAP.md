...
- [ ] **Close the `/api-docs` gaps found on 2026-07-30.** The page is the whole of the consideration `security.md` §4.4 receives for disabling `/openapi.json` on the gateway, and an audit against `routers/chat.py`, `schemas/chat_schemas.py` and `errors.py` found five omissions — everything it *does* say is accurate. In the order they cost an integrator most: **knowledge grounding is undocumented** (`use_knowledge`, `knowledge_collection`, and citations in `X-Knowledge-Sources`, all implemented on both gateway paths); **`temperature`, `top_p`, `n`, `stop`, `tools` and `response_format` are accepted and silently dropped** (pydantic `extra` is the default `ignore`), so a caller setting `temperature: 0` gets 200 and no effect; **a stream failing after the first byte is a 200 carrying an error frame with no `[DONE]`**, which `sse.py` calls "documented for consumers" and is not, along with the wall-clock deadline that arrives the same way; **`usage.prompt_tokens` is always 0 and streaming carries no `usage` at all**; and **five reachable error codes are missing from the table** — `vector_store_unavailable` (503), `runtime_capability_unsupported` (400), `model_not_found` (404), `untrusted_proxy` (400) and the 500 `internal_error` fallback — four of them newly reachable because grounding is. Two smaller corrections: the envelope also carries `type`, and `Retry-After` is set on `quota_exceeded` too. Recorded in [PROGRESS.md](./PROGRESS.md) 2026-07-30
- [ ] **Decide whether `prompt_tokens` should be counted or declared absent.** It is `0` on every response because `_collect` fills only the completion side, so anyone doing cost accounting under-counts the input permanently. Documenting "not reported" is free; counting it means changing what `RouteChatRequest` records, and the quota deliberately meters produced tokens only. The inclination is to document first and count later, which is why this is separate from the item above

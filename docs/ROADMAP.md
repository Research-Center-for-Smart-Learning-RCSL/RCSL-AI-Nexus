...
- [ ] **Reconcile the registry's `loaded` state against what is actually resident.** `loaded` is asserted once at load time and never re-checked, so a row can read loaded while the runtime holds nothing — `qwen7b` did for hours on 2026-07-27. `OLLAMA_KEEP_ALIVE=-1` removes the common cause (Ollama's own timer evicting behind the platform's back) but not the class: a runtime restart, an out-of-band `ollama rm`, or an eviction to make room all leave the same lie. Belongs with `MetricsPort` above, which has the same shape — reading real state back from the runtime
